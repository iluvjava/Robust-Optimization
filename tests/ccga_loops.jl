...
mutable struct CCGAResults
    ...
    return new(Vector{CCGAInnerloopParameters}()) end
end
...
function CCGAInnerLoop(
    ...
    model_fmp = Model(with_optimizer(Gurobi.Optimizer, GUROBI_ENV)); set_silent(model_fmp)
    ...
    for _ in 1:max_iter
        ...
        model_fsp = Model(with_optimizer(Gurobi.Optimizer, GUROBI_ENV)); set_silent(model_fsp)
        fsp = FSP(w̄, d, model_fsp, sparse_vee=sparse_vee)
        Solve!(fsp); push!(lowerbound_list, fsp |> objective_value)
        @info "(FSP Lower, FMP Upper) = $((lowerbound_list[end], upperbound_list[end]))"
...
function CCGAOutterLoop(
    ...
    @assert epsilong > 0 "$context parameter epsilon should be strictly positive but got: $epsilon. "
    ...
    model_mp = Model(HiGHS.Optimizer); mp = MP(model_mp, γ⁺); set_silent(model_mp)
    model_msp = Model(HiGHS.Optimizer); msp = MSP(model_msp, d̂, γ⁺); set_silent(model_msp)
    ...
    for II in 1:outter_max_itr
        OuterCounter += 1
        Results = CCGAInnerLoop(GetGamma(msp), w̄, d̂, ϵ, sparse_vee=false)
...
CCGAOutterLoop(d̂, γ_upper)


# model_mp = Model(HiGHS.Optimizer); mp = MP(model_mp, γ̄); set_silent(model_mp)
# model_msp = Model(HiGHS.Optimizer); msp = MSP(model_msp, d̂, γ̄); set_silent(model_msp)
# PortOutVariable!(mp, :d) do d fix.(d, d̂, force=true) end
# PortOutVariable!(mp, :v) do v fix.(v, 0, force=true) end
# Solve!(mp)
# w̄ = Getw(mp)
...
# for II in 1:10
#     OuterCounter += 1
#     Results = CCGAInnerLoop(GetGamma(msp), w̄, d̂, sparse_vee=false)
#     OuterResults(Results)
#     fig = plot(Results.upper_bounds, label="upper_fmp", marker=:x)
#     plot!(fig, Results.lower_bounds, label="lower_fsp", marker=:x)
#     fig|>display
